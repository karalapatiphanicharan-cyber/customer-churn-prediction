...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Handle categories for one-hot encoding later
        ...
        if 'Contract' in df.columns:
            ...
            df['contract_type_risk'] = 1 # Default to medium risk if missing

        # 5. customer_value_segment
        # Note: qcut depends on the data distribution. For a transformer, we might need pre-defined bins
        # or fit it during fit(). For simplicity and consistency with previous phase:
